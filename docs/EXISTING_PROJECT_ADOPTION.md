...
Note: Artifacts generated by `v1.1.0` (Phase 0.5) lack formal provenance binding and are no longer supported. Re-run `dry-run` to generate a fresh, compatible draft bundle.
...
The provenance receipt is a `TOOLCHAIN_PROVENANCE_BINDING`: it detects ordinary artifact tampering and binds the declared local generator inputs. Because it has no secret key, HMAC, or external signature, it is not cryptographic anti-forgery and must not be described as such.
