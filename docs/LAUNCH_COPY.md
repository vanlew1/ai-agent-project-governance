...
Coding Agent Governance includes an offline demo for denied diffs, stale verification, and successful closure. It complements upper workflows such as Superpowers; it is not an official integration. https://github.com/vanlew1/coding-agent-governance

## X / 中文

上层 AI 编码工作流可以帮助 Agent 规划和施工；这个本地优先运行时独立检查下一步：实际改动是否越界、证据是否仍有效、任务是否可以收口。

Coding Agent Governance 提供离线 Demo：越界 diff 阻断、旧验证失效、成功 Closure。它可与 Superpowers 等工作流配合，但不是官方集成。https://github.com/vanlew1/coding-agent-governance
