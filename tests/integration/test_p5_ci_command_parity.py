...
class P5CIParity(unittest.TestCase):
 def test_workflow_calls_fixed_gate_with_read_only_permissions(self):
  ...
  self.assertIn("python scripts/run_governance_ci.py",text); self.assertIn("contents: read",text); self.assertNotIn("pull_request_target",text); self.assertNotIn("secrets.",text)
